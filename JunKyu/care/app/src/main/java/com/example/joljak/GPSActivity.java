...
public class GPSActivity extends AppCompatActivity implements OnMapReadyCallback,
    ...
    private class ConnectServer extends AsyncTask<String, Void, String> {
        ...
        protected String doInBackground(String[] sId) {
            try {
                String str;
                URL url = new URL("경로");//jsp 주소를 연결
                HttpURLConnection connection = (HttpURLConnection) url.openConnection();
                connection.setRequestProperty("Content-Type", "application/x-www-form-urlencoded");
                connection.setRequestMethod("POST");
                OutputStreamWriter outputStreamWriter = new OutputStreamWriter(connection.getOutputStream());
                String tempLat = Double.toString(prelat);
                String templong = Double.toString(prelon);
                SendMessage = "위도" + tempLat + "경도" + templong;
                outputStreamWriter.write(SendMessage);
                //receiveMessage구현 해야할까?
